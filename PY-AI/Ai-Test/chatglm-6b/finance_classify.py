...
def init_prompts():
    ...
    for _type, example in class_examples.items():
        # print(f'键--》{_type}')
        # print(f'值--》{example}')
        pre_history.append(( f'"{example}"是{class_list} ', _type))
...
if __name__ == '__main__':
    ...
    # device = 'cuda:0'
    device = 'cpu'
    tokenizer = AutoTokenizer.from_pretrained("/Users/yulei/ai/chatglm-6b", trust_remote_code=True)
    model = AutoModel.from_pretrained("/Users/yulei/ai/chatglm-6b", trust_remote_code=True).float()
    model = model.eval()
